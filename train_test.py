...
def train_test(data_folder, view_list, num_class,
    ...
    for train_idx, test_idx in kf.split(np.zeros(len(labels_all)), labels_all):
        ...
        adj_tr_list, adj_te_list = gen_trte_adj_mat(train_data_list, test_data_list, {'tr': list(range(len(train_idx))), 'te': list(range(len(train_idx), len(train_idx) + len(test_idx)))}, adj_parameter)
        ...
        # 计算训练误差
        tr_prob = test_epoch(train_data_list, adj_tr_list, list(range(len(train_idx))), model_dict)
        tr_pred = tr_prob.argmax(1)
        tr_true = labels_train

        train_acc = accuracy_score(tr_true, tr_pred)
        train_f1_weighted = f1_score(tr_true, tr_pred, average='weighted')
        train_bacc = balanced_accuracy_score(tr_true, tr_pred)

        train_accs.append(train_acc)
        train_f1_weighted_scores.append(train_f1_weighted)
        train_bacc_scores.append(train_bacc)

    print("Accuracy for each fold: ", accs)
    print("F1 weighted for each fold: ", f1_weighted_scores)
    print("BACC for each fold: ", bacc_scores)
    print("Train Accuracy for each fold: ", train_accs)
    print("Train F1 weighted for each fold: ", train_f1_weighted_scores)
    print("Train BACC for each fold: ", train_bacc_scores)

    print("Average Accuracy: {:.3f}".format(np.mean(accs)))
    print("Average F1 weighted: {:.3f}".format(np.mean(f1_weighted_scores)))
    print("Average BACC: {:.3f}".format(np.mean(bacc_scores)))
    print("Average Train Accuracy: {:.3f}".format(np.mean(train_accs)))
    print("Average Train F1 weighted: {:.3f}".format(np.mean(train_f1_weighted_scores)))
    print("Average Train BACC: {:.3f}".format(np.mean(train_bacc_scores)))
